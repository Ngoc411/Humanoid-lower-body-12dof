...
4. **State machine** — 3 states, `INIT → RUNNING → FALLEN → INIT`. The 5 s warmup
   lives on the **STM32**, not the Jetson. STATUS is checked in INIT (resume) and in
   RUNNING every frame (so the STM can command a stop).

```
INIT      status==READY ──────────────────────────► RUNNING (reset fall debounce)
          (else: stay, no action out)

RUNNING   status!=READY (STM stop) OR detectFall() ─► resetPolicyState() ► FALLEN
...
```
   ...
   - **STM stop:** STATUS is read every frame in RUNNING; `STATUS=WAIT` → FALLEN
     immediately (the STM can stop the policy any time).
...
|---------|-------|----------|
| WAIT/READY handshake | main_node INIT + RUNNING | READY starts RUNNING (INIT); WAIT stops it any frame (RUNNING→FALLEN); 5 s warmup on STM |
