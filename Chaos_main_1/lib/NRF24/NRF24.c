...
/* ACTIVATE (0x50,0x73): mo khoa ghi FEATURE/DYNPD neu dang bi khoa.
 * Dung probe de tranh toggle nguoc khi MCU reset nhung NRF van con nguon. */
void nrf24_activate_features(void){
	uint8_t feature = nrf24_r_reg(FEATURE, 1);
	uint8_t probe = feature ^ (1 << EN_DPL);
	nrf24_w_reg(FEATURE, &probe, 1);

	if(nrf24_r_reg(FEATURE, 1) != probe){
		uint8_t cmd[2] = { 0x50, 0x73 };
		csn_low();
		HAL_SPI_Transmit(&hspiX, cmd, 2, spi_w_timeout);
		csn_high();
	}
...
void delay_us(uint16_t del_time){
	...
	uint32_t ticks = (uint32_t)del_time * (SystemCoreClock / 1000000U);
...
void nrf24_init(void){
	...
	/* Enable DWT cycle counter used by delay_us() */
